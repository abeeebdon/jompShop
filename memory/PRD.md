...
- **2026-02-28**: **Major restructure for shopper-first marketplace** + production-domain prep at https://www.jompshop.com/. Brand renamed globally Jomp Trade → **Jomp Shop**. Catalog expanded to **50 export-ready African listings** (canonical source of truth `/app/backend/shop_catalog.py`) across 7 verticals: fashion (10), staple-foods (9), beauty (8), home-decor (8), accessories (6), agriculture (5), beverages (4); mix of `riby_dtc` direct-from-Africa and `buyer_local` US-warehouse modes. **Routing**: `/` is now the Shop marketplace landing; `/shop` is an alias; old B2B Landing moved to `/about` (auto-redirect removed). New shopper-first hero (search bar, 7 category tiles, 5-col product grid, Amazon-style top-nav: Shop / Direct from Africa / US In-Stock / About / Become a Seller). New 4-column footer. **Sign-up role chooser** — `/register` now shows a 2-step flow with three role cards (Direct Customer · Reseller/Bulk Buyer · African Exporter); `?role=` deep-link supported (used by "Become a Seller" footer link). User dropdown menu in marketplace header. Login redirects: consumer → `/`, exporter/buyer → `/dashboard`, jompstart_admin → `/admin/credit`, admin → `/admin`. 23/23 iter7 pytest pass + iter6 regression suite still green.
